...
* I couldn't get the project update feature work properly with the way data is fetched from two collections so I decided to remove and add a document to projects collection every time a user updates a project. It is easy to fetch the data from previous project document but when there are many properties assigned to each user, it is practical to delete the previous document and replace it with the updated one. Also I used Firebase v. 8.5 for this project which might have caused some of the troubles that I've had. I made a recipe app with Firebase v. 12 before this project and with that I was able to update docs with updateDoc function. But at least now I know two different ways of updating docs and can assess pros and cons that come with each of these approaches.
